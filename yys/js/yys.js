(function($){
	$.fn.gallery_slider=function(options){
		...
		var _this = $(this),
	  	...
	  	//左侧按钮绑定点击事件
  		this.find(_gallery_item_left).on('click',function(){
			...
			_this.find(_threeD_gallery_item).eq(idx == 1? idx + 2 : idx - _imgNum + 2).removeClass('gallery_right_middle').addClass('gallery_out');
			//当idx值为_imgNum - 2时，执行逻辑
			_this.find(_threeD_gallery_item).eq(idx == 2 ? idx + 1 : idx - _imgNum + 1).removeClass('front_side').addClass('gallery_right_middle');
		})
		//右侧按钮绑定点击事件
		this.find(_gallery_item_right).on('click',function(){
			...
			//当idx值为_imgNum - 2时，执行逻辑
			_this.find(_threeD_gallery_item).eq(idx == 2 ? idx + 1 : idx - _imgNum + 1).removeClass('gallery_out').addClass('gallery_right_middle');
		})
