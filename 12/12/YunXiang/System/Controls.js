...
(function()
{
	...
	*/
	...
	function Button(config)
	{
		...
		This.SetText = function(value)
		{
			This.GetDom().firstChild.rows[0].cells[1].innerHTML = value;
			Base.SetText(value);
		}
	...
	(function()
	{
		...
		function TreeNode(treeView, parentNode, name, text, imgSrc, tag, container, dataSource, height, isLast, hasChildren)
		{
			...
			*/
			...
			{
				if (This.IsExpand()) This.Collapse(); else This.Expand(function(result, error) { if (!result) alert(error) });
			}
			...
			textCell.innerHTML = String.format("<div class='nodeText'>{0}</div>", text);
			...
			function ListHeader()
			{
				...
				This.Create=function()
				{
					...
					for (var i in _columns)
					{
						...
						colDom.innerHTML = String.format("<span>{0}</span>", col.GetText());
						(function(index)
						{
	...
	this.SetText=function(text)
	{
		...
		for(var i in items)
		{
			if(items[i].Text==text)
			{
				...
				{
					dom.firstChild.firstChild.innerHTML = text;
					This.OnChanged.Call();
				}
			}
		}
	...
	this.SetValue=function(value)
	{
		...
		for(var i in items)
		{
			if(items[i].Value==value)
			{
				...
				{
					dom.firstChild.firstChild.innerHTML = items[i].Text;
					This.OnChanged.Call();
				}
			}
		}
...
function Toolbar(config)
{
	...
	This.CreateControl=function(item)
	{
		if(item.Type==undefined || item.Type=="Button")
		{
			...
			dom.innerHTML = String.format("<div>{0}</div>", item.Text, Toolbar.PredefineHeight);
	...
	{
		m_Btns[index].firstChild.innerHTML = text;
	}
