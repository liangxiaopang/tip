...
function CodeBuilder(application) //CodeBuilder
{
	...
	'{PAD}if(!TESTING)\r\n'+
	'{PAD}{{\r\n'+
	'{PAD}{TAB}System.Link("StyleSheet", System.GetUrl("{CSS}"), "text/css");\r\n'+
	'{PAD}}\r\n'+
	'{PAD}\r\n'+
	...
	m_EditPath.GetTextBoxDom().onkeyup = function()
	{
		label4.SetText(
			String.format(
				...
				"代码文件：<span style='color:Blue;'>System/{0}.js</span><br/>" +
...
function Application()
{
	...
	{
		return String.format("System/{0}.js", m_SolutionFile);
	}
	...
	function MainForm()
	{
			...
			dialog.ShowDialog(
				...
				function()
				{
					...
					var postData = String.format(
						...
						"</Data>",
						dialog.GetPath(),
						System.TransferCharForXML(System.RenderJson(data))
					);
					...
					function(xml, text)
					{
						...
						if (ret.Result == "OK")
						{
							...
							try
							{
								...
								var data = System.ParseJson(ret.Data.Solution);
		...
		function Save(completedCallback, errorCallback)
		{
			...
			var postData = String.format(
				...
				"</Data>",
				m_SolutionFile,
				System.TransferCharForXML(content),
				System.TransferCharForXML(code),
				System.TransferCharForXML(m_CssEditor.GetCssCode())
			);
		...
		m_Toolbar = new Controls.Toolbar(
			{
				...
				Items: [
					...
					{ Css: "Developer_BtnNew", Text: "新建桌面", Command: "NewDesktop" },
					...
					{ Css: "Developer_BtnExec", Text: "运行并保存", Command: "ExecuteAndSave" }
				]
			}
		);
				...
				case "ExecuteAndSave":
					{
						function exec()
						{
							...
							try
							{
								System.Test(GetExecFileName(), m_CodeBuilder.RenderCode(), RedirectCssUrl(System.Path.GetDirectoryName(GetCssFileName()), m_CssEditor.GetCssCode()));
							}
							catch(ex)
							{
								alert(ex);
